...
> **Live demo:** [https://myd9.github.io/htmltool3/](https://myd9.github.io/htmltool3/)
>
> **Upstream credit:** This project is a maintained enhancement of [cxq0517/htmltool2](https://github.com/cxq0517/htmltool2), created by **caoxianqi** ([cxq0517](https://github.com/cxq0517)). Many thanks to the original author for publishing the project under the MIT License. The original copyright and license notice are preserved in [LICENSE](LICENSE).
...
## Enhancements in htmltool3

Compared with the upstream project, this maintained version adds:

- PPT-style element selection, dragging, and eight-handle resizing
- Live width/height feedback and proportional corner resizing with `Shift`
- Undo and redo history for text, color, move, and resize edits, with up to 100 states
- `Ctrl` / `Cmd` shortcuts for undo, redo, and save
- Layout-stable text editing that avoids moving elements when edit mode is turned off
- Updated bilingual documentation and a GitHub Pages live demo
...
- PPT-style positioning: drag selected elements and resize them with eight handles; hold `Shift` for proportional corner resizing
- Undo and redo: restore text, color, move, and resize edits with up to 100 history states
- Stable edit mode: only the active text element becomes editable, so toggling edit mode does not change page layout
...
Try the hosted version immediately:

[Open the live editor](https://myd9.github.io/htmltool3/)
...
5. Click empty space inside an element to select it
6. Drag the selection box to move it, or drag a handle to resize it; hold `Shift` while dragging a corner to preserve the aspect ratio
7. Click the floating color button to open the color panel
8. Click "Save" to download `original-name-edited.html`
...
| `Esc` | Close the color panel, then clear the selection |
...
- Adding, deleting, and reordering elements are intentionally unsupported
- Move and resize edits are stored as inline geometry styles, so highly constrained flex/grid layouts may react differently from free-positioned presentation elements
...
This repository retains the original MIT copyright notice for caoxianqi. See [cxq0517/htmltool2](https://github.com/cxq0517/htmltool2) for the upstream project.
